    ...
    <div class="row justify-content-center">
        <div class="col-lg-12">
            ...
            <h2 class="text-center mb-4 text-dark fw-bold">
                <i class="fas fa-briefcase text-primary"></i> Gestión de Personal y Plazas
            </h2>

            <!-- Botón centrado -->
            <div class="d-flex justify-content-center mb-4">
                <a href="{{ route('personalplazas.create') }}" class="btn btn-primary btn-lg shadow rounded-pill px-4">
            ...
            <div class="table-responsive shadow rounded" style="border-radius: 20px; overflow: hidden; max-width: 1100px; margin: 0 auto;">
                <table class="table table-hover table-borderless align-middle">
                    <thead class="bg-dark text-white">
                        <tr>
                            <th style="width: 30%;">Nombramiento</th>
                    ...
                    <tbody class="bg-light">
                        ...
                        <tr class="hover-shadow">
                            <td class="fw-bold">{{ $pe->tiponombramiento }}</td>
                            <td>{{ $pe->plaza->idplaza }}</td>
                            <td>{{ $pe->personal ? $pe->personal->nombres . ' ' . $pe->personal->apellidop . ' ' . $pe->personal->apellidom : 'N/A' }}</td>
                            <td class="text-center">
                                <a href="{{ route('personalplazas.edit', $pe->id) }}" class="btn btn-outline-primary btn-sm rounded-pill">
                                    Editar
                                </a>
                            </td>
                            <td class="text-center">
                                ...
                                <button type="button" class="btn btn-outline-danger btn-sm rounded-pill delete-btn" 
                                        data-id="{{ $pe->id }}" data-action="{{ route('personalplazas.destroy', $pe->id) }}">
                                    Eliminar
                                </button>
                            </td>
                            <td class="text-center">
                                <a href="{{ route('personalplazas.show', $pe->id) }}" class="btn btn-outline-info btn-sm rounded-pill">
                                    Ver
                                </a>
                            </td>
                        </tr>
