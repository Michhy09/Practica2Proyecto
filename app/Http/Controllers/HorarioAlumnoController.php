...
class HorarioAlumnoController extends Controller
{
    ...
    public function pdf()
    {
        ...
        // Verificar los grupos recuperados
        if (empty($grupoIds)) {
            dd('No se encontraron grupos para el alumno.');
        }
    
        // Cargar todos los horarios de los grupos relacionados
        $grupoHorarios = GrupoHorario::with([
            'lugar',                          // Lugar asociado al horario
            'grupo.materiaAbierta.materia',   // Materia asociada al grupo
            'grupo.personal' ,
            'grupo.periodo' 
        ])
        ...
        if ($grupoHorarios->isEmpty()) {
            dd('No se encontraron horarios para los grupos.');
        }
